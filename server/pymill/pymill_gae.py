...
class Pymill():
    ...
    def __init__(self, privatekey):
        ...
        """
        self.auth_header = {"Authorization": "Basic %s" % base64.b64encode("%s:" % privatekey)}
    ...
    def _apicall(self, url, params=(), cr="GET", ch=None):
        ...
        """
        pars = urllib.urlencode(dict(params))

        if params is not () and cr != "DELETE"and cr != "PUT":
            cr = "POST"

        if(ch):
            ch.update(self.auth_header)
            r = urlfetch.fetch(url, method=cr, payload=pars, headers=ch, deadline=60, validate_certificate=False)
        else:
            r = urlfetch.fetch(url, method=cr, payload=pars, headers=self.auth_header, deadline=60, validate_certificate=False)
